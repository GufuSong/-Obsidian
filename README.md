ww> Unreal Engine 材质渲染模块笔记 ,  作者松谷福 ,  QQ :  2148103921 .  反馈错误联系我 ,  好人被从天而降的金砖砸晕 .
...
- Default Lit 正常光照
- subsurface 透射  
	- `Opacity` :  透光度 ;  控制光穿过表面 ,  在内部反射传播 .  控制表面与内部元素的透光率的 ,  便是此属性 .  透光率越低 ,  Opacity越高 ,  呈反比 .  注意 ,  透光率影响的是光穿过物体的距离 ,  与物体本身的尺寸无关 ;
	- `Subsurface Color` :  光透过的颜色 / 光能透过的颜色 ( 黑色即为没有光能透过 ) .
...
- `Output Depth and Velocity`:  输出像素深度信息 .  勾选后可以计算每个像素的深度信息 .
...
- **`Specular`镜面反射**
	- 整表面对直接光源（如平行光、点光）的镜面反射强度。值越高，高光越亮；值为0时，表面可能显得完全粗糙（无镜面反射）。

- **Anisotropy**：  
    控制高光的各向异性，可拉长高光形状（如金属拉丝效果）。
>半透明物体想使用法线属性需要在半透明光照模型中使用Driectional.


- `Roughness` :  粗糙度属性 
	- 在非光线追踪情况下, UE计算反射是烘焙的反射球 + 屏幕空间反射SSR .  所以并不准确 .  因此物体反射在材质预览窗口里与场景里效果不同是常有的情况 .  在没有使用`Sphere Reflection Capture`反射球的情况下 ,  物体反射只会反射出屏幕空间出现的物体 .  而反射球可以帮助我们捕捉范围内的物体 ,  并烘焙一张低分辨率的贴图 .  令屏幕空间外的像素也能产生一些被反射的效果 .
	- 这是主要控制高光形状的参数。值越低，高光越锐利（如光滑塑料）；值越高，高光越扩散（如磨砂表面）。
...
- `Tessllation Multiplier`: 曲面细分系数

**六. Advanced 高级设置 :**

- `Responsive AA`:  响应式抗锯齿 .
- `Trandlucency Pass`:  半透明通道计算时间/渲染顺序 .   原`Render After DOF`,  当为After DOF时,  材质有Custom Stencil信息 .
- `Allow Custom Depth Writes`:  准许自定义深度写入 .

>任何类在参与渲染时可以在Details面板中搜索Shadow 进行详细的阴影设置 .
...
- `Fade Start Delay` :  从贴花生成到 开始淡入 时的延迟时间 .
- `Fade in Duration` :  贴花从 完全透明到完全显示 所需的时间 .
- `Fade Duration` :  总生命时间  .  注意 ,  与`Fade in Duration`有一定耦合性 .
- `Fade in Start Delay` :  同一参数的不同命名 ,  历史残留问题 .
...
### 1.17 环境纹理 / Environment Map

**一. 场景组件`Scene Capture Cube` :**
...
### 2.4 Texture Sample Type

**一. 基础设置与调试 :**

**1. SRPG 伽马矫正 :**
- 令纹理整体变暗 ,  饱和度变高 .  暗部细节会相较于原图片更多 .  为了应对人眼对亮部更敏感 ,  对暗部更迟钝 .  此功能会增加暗部的细节信息 .
- 当弃用SRPG功能时 ,  会无法使用一些采样种类 .
- 默认情况下 ,  8位贴图只能存储(0~255)大小的信息颜色 ,  SRPG 可以更改这些数值的分布规则 ,  默认状态下(0~255)是平均分布在这张贴图上的 ,  SRPG则是令暗部的数据密度更多 ,  亮部的数据密度更少 ,  达到暗部细节纹理的增多 .

**2. Sample Type :**
- 默认情况下 ,  8位贴图只能存储(0~255)大小的信息颜色 ,  
  


### 2.5 Virtual Texture 虚拟纹理

>如果想要启用此功能 ,  需要现在Project Setting 中启用此功能 .

**一. 普通纹理和虚拟纹理的区别 :**

**1. 普通纹理的LOD :**
- 根据光栅化后 ,  类投射在屏幕上的比例 / 摄像机距离屏幕的物体的远近去分配纹理采样时采样的纹理层级 .

**2. 虚拟纹理 :**
- 虚拟纹理在普通纹理的LOD的框架上 ,  将其分成不同的 Tile/Page 单元(将贴图分成128x128块) ,  引擎会根据Actor的UV决定出现在屏幕上渲染区域的纹理被加载至内存中 ,  将被加载的区域会储存为Page Table ,  需要被加载到部分是灰色的 ,  不需要被加载的部分是白色的 .  最终这些数据会变成一张最终被使用到的纹理图片 .  

>启用Virtual Texture的方法为在纹理界面启用`Virtual Texture Streamin`
>或右键直接Convert to Virtual Texture .

>注意, 虚拟纹理与普通纹理的采样方法不同 ,  谨慎更换 .
...
### 3.21 `SmoothStep`平滑过渡
>在感觉上 ,  此数学函数会将暗部增强 ,  亮部加量 .  原理上 ,  此数学函数会以中间数为原点 ,  值域两端开始构建一条三角函数 .  分别增强亮暗部 .  类似RemapRange节点 .  但是此数学节点比较它灰部更平滑 .  机能消耗与Power相接近 .
...
**1. 算法特点 :**
- 计算方式与Remap非常相似 ,  但结果更平滑 ,  适用于替代Contrast提高特定区域的对比度 ,  制作溶解 .

### 3.22 `Bump Offset`视差映射

**一. 节点使用 :**

**1. 参数列表 :**
- `Coordinate`:  输入UV坐标 .
- `Height` :  深度 ,  0.5为标准深度 ,  即为不偏移 ,  低于0.5的凹进去 ,  高于0.5的凸出来 .
- `HeighRatioInput` :  深度系数 ,  越小越靠近基准平面 ,  越大最高点越远离基准平面 .  注意, 此系数算法会将`(Height *2)- 1` .

**2. 使用须知 :**
- 此节点属于略高消耗的节点 ,  低端平台慎用 .


### 3.23 `TextureVariation`纹理变化
>随机UV ,  常用于地皮,  草皮等 .
...
### 4.5 `SkyAtmosphereLightDirection`光源方向

- 作用 :
	- **ight Index** 参数，对应于场景中**第几个**被指定为 Sky Atmosphere 的 Directional Light。
	- Unreal 支持**最多两个** Sky Atmosphere 光源（一般是太阳和月亮），Light Index 分别为 0 和 1.
	- 节点输出的是世界空间下该光源的方向，常用于模拟大气散射、天空色、太阳光等材质效果。
...
- 使用须知 :
	- 在`Deferred Decal`中 ,  此节点返回被贴花覆盖区域的像素世界坐标 .
...
### 6.10 `Spline Thicken`加厚样条线

**一. 函数使用 :**

**1. 参数列表 :**
- `WidthBase`:  前方宽度(靠近摄像机) .
- `WidthTip`:  后方宽度(原理摄像机) .
- `Expand U or V UV Channel`:  反转切线方向 .

### 6.11 `WorldPositionBehindTransLucency`获取背景像素的世界位置信息
...
>关于顶点着色的感性理解 :
>	<桥梁下的河流> :  桥梁下的河流在现在与从前的时间维度下 ,  现在的水在时间下是从前的水经过流动到达的 .  假设桥梁下的空间是静止的 ,  想要静止的空间呈现流动的趋势 ,  则只要从时空下向后依次表达 .  在视觉上呈现水在向前流的势 .
...
#### `ResolvedView.` 字段
>LWC Large World Coordinates 是针对大规模世界场景（比如地图超越几十公里）引入的一套 **双精度支持** 系统，旨在解决传统浮点精度不足带来的“抖动”、“错位”等问题。 传统的 32 位浮点（float）在世界坐标超过几公里后精度下降，表现为物体抖动、顶点错位，尤其在高精度需求项目中更明显 .  

##### `ResolvedView.WorldToClip`世界坐标->裁剪坐标
>常规用法 :`mul(float4(In.xyz, 1) , ResolvedView.WorldToClip)` 将世界坐标矢量变换至裁剪坐标 .  注意仅支持齐次坐标系 .

##### `ResolvedView.TranslatedWorldToClip` 处理镜头偏移后->裁剪坐标
>考虑 LWC（大世界坐标）偏移后的投影矩阵，可用于高精度场景中将世界位置正确投影到屏幕空间

##### `ResolvedView.WorldCameraOrigin` 相机在世界坐标中的位置

##### `ResolvedView.AtmosphericFogSunDirection`太阳光方向
...
##### `LWCToFloat()` 将高精度LWC转换至float
>`ResolveView`会考虑LWC系统并输出双精度数值 ,  因此需要此函数显示转换至float .


##### `ViewportUVToBufferUV(float2 $)`屏幕像素坐标转换至BufferUV
>在实际渲染中, Buffers会比视口更大, 或包含多视图等不确定因素. 因此如果直接使用ViewportUV去采样Buffers, 可能会导致取错区域或导致缩放变形等.

##### `InterleavedGradientNoise`()伪随机函数
>`InterleavedGradientNoise(ScreenPos.xy, float(View.StateFrameIndexMod8));`它通过像素位置 `(x, y)` 和当前帧索引生成一个统一分布在 `[0, 1)` 区间的随机值 . 相比白噪声，它在每个 3 × 3 像素块中会更均匀分布，减少 clumping，对 TAA 效果提升显著
...
# Ⅲ. Shading Model 渲染模型



## $. Default Light 默认光照模型
...
$$
L = 漫反射颜色\times漫反射系数+高光颜色\times高光系数^{光斑集中度}
$$
$$
半程向量H = normalize((LightPosition-PixelPosition)+(CameraPosition-PixelPosition)
$$
- 在传统的光照模型中 ,  漫反射系数多为灯光方向点乘向量 .  注意 ,  点积系数最小值为0 ,  我们要钳制他的数值大小 .  
$$
L=Color_{diff}\times(LightDir\cdot Normal)+Color_{spec}\times(H_{半程向量}\times Normal)^{光斑集中度}$$
- 但是 ,  但我们提高 光斑集中度 时 ,  高光的范围缩小 ,   但亮度没有提升 .  因此 ,  我们新增高光系数 ,  并将高光与漫反射耦合 .
$$
L=\frac{Color_{diff}}{\pi}\times(LightDir\cdot Normal)+Color_{spec}\times(H_{半程向量}\times Normal)^{光斑集中度}\times(LightDir\cdot Normal) \times \frac{光斑集中度+8}{8\pi }
$$
>光斑集中度通常在 0 - 128 .  
- 此时 ,  我们仍然可以把公式视为 :  漫反射部分 + 高光部分 .  由于两者都乘以了 :  $(LightDir\cdot Normal)$ , 我们可以把他提取出来 .

$$
L = \left (  \frac{Color_{diff}}{\pi} +Color_{spec}\times(H_{半程向量}\times Normal)^{光斑集中度}\times \frac{光斑集中度+8}{8\pi }\right ) \times(LightDir\cdot Normal)
$$
- 将前半部分封装为 $f_{BRDF}$ .
$$
f_{BRDF} = \frac{Color_{diff}}{\pi} +Color_{spec}\times(H_{半程向量}\times Normal)^{光斑集中度}\times \frac{光斑集中度+8}{8\pi }
$$
- 最后得到 :
$$
L=f_{BRDF}*(LightDir\cdot Normal)
$$

### 2. GGX 法线分布函数 

- 在$f_{BRDF}$ 函数中 ,  把表达高光范围的两个系数提取出来 ,  称呼为NDF(Normal Distribution Function) .  

$$
NDF = (H_{半程向量}\times Normal)^{光斑集中度}\times \frac{光斑集中度+8}{8\pi }
$$
- 于是我们便得到了`Blinn-Phone` 法线分布模型 .
...
$$
NDF_{GCXTR}(n,h,\alpha)=\frac{\alpha^2}{\pi((n\cdot h)^2(\alpha^2-1)+1)^2}
$$
- $\alpha$ 表示粗糙度 ,  模型中使用他的平方是避免线性梯度带来的美术问题 .  $n$ 是法线 , $h$ 是半程矢量


### 3. 微表面理论

- 理论上 ,  只有摄像机完全处于物体反射光线上 ,  才能观察到高光 .  之所以能存在半程向量的范围 .  本质是物体是由很多微表面构成的 .


- 假设**把每一个像素看作一块凹凸不平的面** ,  这些面会产生遮挡 ,  导致光线照射不出去, 也接收不到光线 .  这导致在模拟高光渲染时 ,  每个像素反射的高光强度并不是规律的 ,  在像素与像素之间 ,  高光的强度也并不是均匀的 .
- 因此 ,  我们加入微表面函数 $V$ , 便得到 :
$$
f_{BRDF} = \frac{Color_{diff}}{\pi} +Color_{spec}\times NDF\times V
$$
...
- $v$ :  任意方向 .  $n$ :  法线方向 $k$ :  经过经验计算后的粗糙度 .

- 由于微表面理论会屏蔽入射光 ,  也会屏蔽反射光 .  因此要使用光线方向与视线方向进行两次运算
- 在虚幻引擎默认的PBR 中 ,  经验参数k的计算方法为 :
$$
k = \frac{(Roughtness+1)^2}{8}
$$
  
- 最终 ,  结合 k 与 `Schlick - GGX`求出虚幻引擎最终位平面相互屏蔽的比率 .
$$
G(l,v,h)=G_1(LightDir)G_1(CameraDir)
$$

### 4. 菲尼尔方程

- 把光视为一根线 ,  入射角越垂直于面 ,  则光线就越容易击穿面并向下进行折射 .  反之 ,  入射角越倾斜 ,  就越难以透过平面 .  而是会反射至另一个方向 .
- 观察到视觉现象便是 :  反射的光线会更亮 .  而折射的光线会透过平面进入到物体内部传播 ,  所以会更暗 . 
- 因此 :  总结规律 ,  入射角越倾斜 ,  反射的光线就越多 .  折射的光线就越少 .  入射角与直 ,  折射的光线越多 ,  反射的就越少 . 
![[ys1.png]]
- 如图所示 ,  图2观察到的高光要比图1 的更亮 .  我们把菲尼尔函数$F$添加值$f_{BRDF}$ 中 ,  便得到 :
$$
f_{BRDF} = \frac{Color_{diff}}{\pi} +Color_{spec}\times NDF\times V\times F
$$
...
- $F_0$ :  经验参数 ,  平面最基础的反射率 .  常见范围为0.02 - 0.05 之间 (线性值) . 
- $h$ :  半程向量 .
- $v$ :  视角方向 .


>----$\text{BRDF}_{env}$ 的实现原理有些难懂 ,  但大致上是当你看向一个物体时，计算它表面究竟该反射出多少天空和环境的光” .  但是实时渲染中，真正的物理积分计算太慢了 .  虚幻引擎（最早在 2013 年提出，后来成为全行业的标准）用了一招绝妙的查表作弊法（Split Sum Approximation，分离求和近似）来实现这个效果。
>----引擎的开发者在做游戏之前，就用超级计算机把所有 `NoV` 和 `Roughness` 组合下的复杂微积分答案算好了，画成了一张 2D 图片（叫做 PreIntegratedGF 贴图 / LUT）.
>----在游戏运行时，GPU 不需要算微积分，只要把 `NoV` 当作 X 坐标，`Roughness` 当作 Y 坐标，直接去这张图上吸取颜色就行了。取出来的红色通道（`r`）赋值给 `A`，绿色通道（`g`）赋值给 `B` .

**二. $F_0$ 经验参数的取值方法 - Specular :**
...
- 当存在参数输入时 ,  当输入参数为 0.5 时 , $F_0$ 取值为 0.04 为1时则取值0.08 .
...
>一句话解释漫反射与高光之间的关系 .  漫反射越强烈 ,  高光越弱 .  他们是线型差值的关系 .

$$
k_d + k_s = 1 \to 
f_{BRDF} = k_d\frac{Color_{diff}}{\pi} +k_s\times Color_{spec}\times NDF\times V\times F
$$

**一. Material Graph 中 Metallic 对能量守恒的耦合 :**

- `Matallic` 主要影响的便是此参数 ,  $k_s = Metallic$ ,  大致是以下关系 :
...
### 6. 现代引擎的反射率方程  Cook - Torrance 反射率方程

$$
Lo​=Le​+∫_\Omega ​f_r​⋅L_i​⋅(ω_i​⋅n)dω_i​
$$

>$∫_Ω$​  对“所有方向”求和 - $L_O$  出射光亮度（你最终看到的)  - $L_E$ 自发光- $f_r$ - BRDF 反射率函数- $L_i$  入射光- $(ω_i​⋅n)$  余弦项 ,  入射光的衰减 .- $dω_i​$ 微小方向权重

- 可以吧这个公式大致理解为 :  自发光 + 反射光 .  其中反射光 = 入射光 x 衰减 x 反射率 .

**一. $f_r$ BRDF 反射率方程**
$$
\int_{\Omega }^{} \left ( k_{漫反射差值系数}\frac{DiffColor}{\pi} +k_{高光差值系数}\frac{法线分布函数\times微表面方程\times 菲尼尔方程}{4(CameraDir\cdot normal)(lightDir\cdot normal)}  \right ) \times 光源颜色(lightDir\cdot normal) + Emsiisve
$$
...
## $. Single Layer Water 单层水材质

>虚幻引擎中的 **Single Layer Water (单层水)** 渲染模型是一种在计算性能和物理真实感之间取得绝佳平衡的方案。它不需要昂贵的真三维体积光线步进（Ray Marching），而是通过**深度读取**和**解析积分**来模拟水体的光学特性。

**一. 使用代价 :**

- 当屏幕上出现`Single Layer Water` 材质的像素时 ,  `Light Pass` 会渲染两次此像素 .  一次渲染带有水面的版本 ,  并计算出水面的高光反射 .  并单独开辟一份显存 ,  储存计算结果 .  而另一次计算被水面覆盖的像素 ,  计算没有水面覆盖时 ,  原本的物体的BRDF信息 .  并存储到另一块内存中 .
- 这就引出了游戏引擎底层优化最经典的一句行话：**“拿空间换时间”**（用额外的内存，去省下成千上万倍的计算时间）.  这样做可以避开真实的光线步进体积算法 ,  尽可能实现更好的效果 .
- 第一次计算（算水底）,  第二次计算（算水面）.  对于这一个像素而言，GPU 确实在这里打了两遍光，跑了两次昂贵的 PBR 物理光照代码 .  但是避免了真实的体积算法 .  否则一束光穿过屏幕像素进入水中，程序需要在一个 `for` 循环里，把光切成几十上百个小段。每一段都要算一次吸收、一次散射、一次灯光。**一个像素的计算量是现在的 50 到 100 倍 .** 别说手机，顶级显卡都会冒烟。


- `Single Layer Water`本身属于半透明材质 ,  实际的计算过程在Light Pass 之后 ,  这正是虚幻引擎**延迟渲染（Deferred Rendering**名字的由来——它把最复杂的着色和合成延迟到了打光之后 .
	- `BasePass` (画水体与打底稿) 还会把水面波浪的**法线 (Normal)** 和 **粗糙度 (Roughness)** 像填表一样，写进屏幕的 **`GBuffer`** 里 .
	- `LightPass` 虚幻的标准光照系统登场 ,  深邃的水面上，出现了波光粼粼的太阳反光 .  并储存进缓存 .
	- `SSR Pass` 擎根据屏幕上已经画好的画面，算出水面应该倒映出什么东西 .  并储存进缓存 .
	- 最后 ,  才开始最终合成 ,  读取缓存渲染为最终画面 .  
...
- 核心思路如下 :
$$
L_{final}=L_{scatter}+(L_{background}\cdot T)+L_{reflect}
$$

$$
L_{最终颜色}=L_{散射}+(L_{背景光 }\cdot T)+L_{水面反射}
$$
...
>光穿过水到达水底（或者从水底反射回眼睛）时 ,  还剩下多少能量 ?  光在水中会被**吸收 (Absorption)** 和 **散射 (Scattering)** ,  两者相加称为**消光系数 (Extinction Coefficient)** .  

- 代码实现 :

```c++
const float3 OpticalDepth = ExtinctionCoeff * BehindWaterDeltaDepth;` `float3 Transmittance = exp(-OpticalDepth);
```


- 数学实现 :

$$T = e^{-(\sigma_s + \sigma_a) \cdot d}$$
- $\sigma_s$ (散射系数) 与 $\sigma_a$ (吸收系数) :  `ScatteringCoeff` (材质节点引脚0) 和 `AbsorptionCoeff` (材质节点引脚1) .  
		- **$\sigma_s$ (散射)**：水里泥沙/微粒把光**弹射到你眼睛里的能力**。值越大，水越像牛奶。
	    - **$\sigma_a$ (吸收)**：水把光**直接“吃掉”的能力**。被吃掉的光就永远消失了。
	- $d$ (光穿过的距离 / 水深) : 光在水里走过的距离 .  对应源码`BehindWaterDeltaDepth`
	- $e$ :  2.71828 .


### $L_{scatter}$ 水面散射求解公式

>**水体内部单次散射积分公式** 太阳光和天空光照进水里，水体本身呈现出的颜色（即浑浊感和体积感）是怎么算出来的？传统做法是沿着视线一段段累加光照，但虚幻引擎直接用微积分算出了整段视线的散射总和。
...
- 数学积分 :
$$
L_{scatter} = L_{lin} \cdot \frac{\sigma_s}{\sigma_s + \sigma_a} \cdot (1-T)
$$

>使用 `Single Layer Water`必须添加 `Single Layer Water Material` .

- 常项解释 :
	- $L_{Li}$ 入射光强 .  对应源码为 :
	- ```c++
AmbScattLuminance + SunScattLuminance * DirectionalLightShadow;
	  ```
	- $\sigma_s$ (散射系数) 与 $\sigma_a$ (吸收系数) :  `ScatteringCoeff` (材质节点引脚0) 和 `AbsorptionCoeff` (材质节点引脚1) .  
		- **$\sigma_s$ (散射)**：水里泥沙/微粒把光**弹射到你眼睛里的能力**。值越大，水越像牛奶。
	    - **$\sigma_a$ (吸收)**：水把光**直接“吃掉”的能力**。被吃掉的光就永远消失了。
...
- 积分解释 :
	- $\frac{\sigma_s}{\sigma_s + \sigma_a}$ (散射占比 / 单次散射反照率)  :  光线的幸存比例 .  假设水一共拦截了 100 份光（总消光），其中有 30 份被吸收吃掉了，70 份被颗粒弹走了。那么这个比例就是 $70 / 100 = 70\%$。它决定了在所有被水体拦截的光里，**有多少比例能变成可见的散射光**。
	- $e^{-(\sigma_s + \sigma_a) \cdot d}$ (透射率 Transmittance) ,  **光的“通关率”** .  经历了厚度为 $d$ 的一路损耗后，还有百分之几的光能安然无恙地穿透水体 .  
	- $(1 - e^{-(\sigma_s + \sigma_a) \cdot d})$ (被拦截的比例) :  既然 $e^{-...}$ 是穿过去的“通关率” ,  那么 $1 -$ 通关率，就是**被水体拦截下来的光的比例**  .  


### $(L_{background}\cdot T)$ 水底渲染公式

>在计算水面着色前 ,  引擎会先画场景里所有不透明的物体 ,  通常使用默认光照模型 .  画完每个像素后 ,  会储存在GBuffer中 .  等轮到渲染时 ,  引擎会根据材质中的`Refraction引脚` 计算基础扭曲的强度 ,  `Normal引脚` 计算波浪扭曲的形状 ,  `Specular 引脚` 决定阳光射入水中的方向 , 计算出折射 ,  并根据此折射信息 ,  采样提前计算好的不透明物体 .

>除了美术能控制的上述 3 个材质引脚外 ,  引擎在底层还会自动根据**水体厚度 (Water Thickness)** 来强制干预折射：引擎会自动计算水深 ,  如果水深不足 30 个单位 , 它会强制把刚刚算好的折射扭曲量（`BufferUVDistortion`）按比例缩小，甚至在极浅的地方降为 0 .这就是为什么你无论怎么调大法线和折射率，水交界岸边的地方看起来过渡依然很自然的原因 .
```c++
BufferUVDistortion *= saturate((PixelSceneDepth - WaterDepth) * 1.0 / 30.0f);
```


**1. 计算 $(L_{background}\cdot T)$ :**

- 引擎通过折射 扭曲采样了提前计算好的BRDF信息 ,  就得到了基础颜色 .
$$
L_{background} =基础颜色\times ColorScaleBehindWater \times 下行透射率 \times 上行投射率
$$

- 其中 ,  `ColorScaleBehindWater` 是材质节点的引脚 .
- 下行透射率 虚幻引擎使用了经验函数 ,  
```c++
...
>虽然它和上行透射率（$T$）用的都是同一个物理定律（比尔-朗伯定律 $e^{-\text{阻力} \times \text{距离}}$），但**光走过的距离算得不一样**。引擎直接拿**水面当前的绝对高度（Z值）** 减去 **水底物体的绝对高度（Z值）**。这省去了极其复杂的 3D 射线求交计算。

- 最后 ,  因为光走到了底 ,  又穿过水底反射回来 ,  在乘以T即可 .


### $L_{reflect}$ 水面高光反射合成公式

>通过屏幕空间反射(SSR) 和 环境天光混合捕捉 ,  并利用菲涅尔效应 .
...
$$L_{reflect} = (\text{SSR} + L_{env} \cdot (1 - \text{SSR}_{alpha})) \cdot \text{BRDF}_{env}$$

- SSR(屏幕空间反射颜色) :  屏幕上看不见，就反射不出来 .
- $L_{env}$ (环境反射颜色) :  代码里早前通过 `CompositeReflectionCapturesAndSkylight` 算出来的 `Reflection`（此时还没乘 BRDF）,  它是 SSR 的超级备胎 .  当 SSR 因为超出屏幕而找不到反射物时 ,  系统就会用天空的倒影来顶替 .
- $\text{SSR}_{alpha}$ (SSR的置信度/遮罩) :  这是一个 0 到 1 之间的值 ,  代表引擎对当前 SSR 反射结果的自信程度 .
- $\text{BRDF}_{env}$ (环境光双向反射分布函数) :  这是菲涅尔定律（Fresnel）和粗糙度（Roughness）的结合体 .  
...
```c++
float3 Lerp3Color (float Alpha){
    
float3 R = float3(0,0,1.5);
float3 G = float3(0,0.5,0);
float3 B = float3(1,0,0);
//float Alpha;
return lerp(   lerp(R,G,saturate(Alpha*2))   ,B,saturate(Alpha*2-1));
}
```

**一. 基础讲解 :**
- 根据Alpha的不同输出不同色相的颜色权重 .
![[interactive_lerp.html]]

- 详细见此文件 .
  
  **1. 分权规则 :**
- 绿色贯穿全局 ,  为0.5时最高 .  0.5之前蓝色 , 0.5之后绿色 .
...
- UV坐标是浮点型二维矢量 .  因此可以根据某一分量的数值对World Position Offset进行线性调整 .

**二. 实践方法 :**

- 在DDC软件编辑顶点UV属性时 ,  可以按需求将顺序性的顶点的UV坐标的某一分量调整为0 ;
- 渲染过程中 ,  编写Vertex Shader时 ,  读取Position UV Vector 的分量值  乘以 偏移的矢量即可到达需求 .


### 3.3 通过顶点偏移实现面朝向摄影机的功能 

>给定一个由四个顶点构成的面(矩形) ,  根据调整其中两个相邻顶点的位置令此四边面的法线正对Camera的原点 .

**一. 理论阐述 :**
- 只需要求出原点指向摄影机的矢量与顶点切线叉乘 ,  将此叉乘值与需要被偏移的顶点position属性偏移即可 .
...
**1. 数学意义 :**
- 整体呈现三角形 .
- 其中 ,  $x,y$决定三角形尖端的坐标位置 ,  $k$决定函数x轴向的斜率 ,  $h$决定函数y轴向的高度缩放 ,  此时峰值变换为$h\cdot y$ .
$$
f(a)=h\;(y-k\;|a-x|)
$$
>a 默认输入坐标系(-∞,+∞)
...
**1. 图形意义 :**
- 输出完美锯齿状的图案 .  常用于呼吸灯 ,  棱角清晰的粗细变换等 .

**二. 数学形式 :**

| 变量    | 数学意义              | 图形影响      | 默认值示例  |
| ----- | ----------------- | --------- | ------ |
| **A** | 波形的最大振幅（峰峰值）      | 垂直缩放波形高度  | A=1    |
| **f** | 波形的频率（f=1/Tf=1/T） | 水平压缩/拉伸周期 | f=1 Hz |
| **ϕ** | 波形的水平平移量          | 左右移动波形起始点 | ϕ=0    |
| **D** | 波形的垂直平移量          | 上下移动整个波形  | D=0    |
...
- 可以产出首尾连接的等腰三角形函数 ,  用于闪电的细节粗细变换 .
- 注意 ,  只有$A=2$时 ,  三角形才可以首尾连接 .

**2. 数学图表 :**
[等腰三角形锯齿波 | Desmos](https://www.desmos.com/calculator/wszavwb93g?lang=zh-CN)
...
**2. 实现逻辑 (线段顶点之一为原点) :**
- 以原点为线的一端 ,  连接一条线 . 
- 计算顶点矢量与线矢量的点积 ,  求出顶点矢量映射在向量上的长度 .
- 将长度乘以单位化的线矢量即可求出顶点矢量与线的垂线的交点 .
- 计算顶点矢量与垂线的交点矢量之间的距离即可 .

**3. 自定义线段顶点位置 :**
- 经过验算 ,  线段的起始点不影响点积结果 ,  只需要将线段的单位化向量乘以点积结果即可 .


## 六. 使用Niagara进行流体模拟解决方案
...
**1. 总体介绍 :**
- NS方程是流体力学重要的方程之一 ,  本质是流体微团的动量方程 .  核心为满足牛顿第二定律 $F=Ma$ 
...
**1. 作用 :**
- 每帧插值可以令速度更符合阻尼效果 ,  令力看起来更自然 .
- 类似引擎的Drag等均为插值运算 ,  尽管有些抽象 ,  但他可以解决很多需要平滑属性过渡的问题 .

**2. 实现 :**
- `Velocity = lerp(上一帧移动速度 , 未处理的原始快速度 , alpha)`
- 当alpha小于0.5 时 ,  每帧的移动速度只会依据原始速度进行小幅度的变化, 有阻力效果 ,  看起来更加的自然 .
...
**1. 定义 :**
- 圆 ,  在平面上是固定点 (圆心) 等于固定距离 (半径 ) 的所有点的集合 .
...
**三. 深度精灵算法 - depth sprite :**

**1. 什么是深度精灵 ?**
- 在二维点精灵或四边形中模拟真实球体深度，使它们在遮挡和排序中表现得像真实3D球，而非平面圆形。
https://www.desmos.com/3d/avelnkgdxe
...
float Blend(float d1, float d2, float k)
	{
		...
		return d2 = lerp(d2, d1, h) - k * h * (1.0 - h);
	}

//当 d2 - d1 = 0 时 ,  
//当 d2 - d1 = k 时 , 代表d2还没有重合形变,取d2,d2不发生特殊,d1变化.
//当 d2 - d1 = -k 时, 代表d1还没有重合形变,取d1,d1不发生变化.
```

**一. 二维 SDF - Signed Distance Function - 有符号的距离函数 :**

**1. 基础介绍 :**
- SDF输入形状的参数 ,  以及平面点的坐标 .  输出点到边缘形状的距离 .
- 距离是有符号的距离 .
- SDF不是一个统一的函数 ,  而是求顶点至几何体距离的函数的统称 ,  在实际使用中 ,  我们需要根部不同的场景编写不同的程序 .

**2. 几何体的表示形式 :**
- 显式表示 ,  使用带有数据的顶点组成三角面 或 四边面来直接表示几何体 .
- 隐式表示 ,  不会直接描述几何体的形状 ,  如通过像素着色与特殊算法实现的几何体 .

>SDF本质就是隐式表达的几何体 ,  SDF通过对空间中周边顶点的判断所计算出的数值来间接表现自己的形状 .
...
### Intro
---
- 基于基本原始体（与网格或神经网络相对）的有符号距离场（SDF）建模的基本构件之一是平滑最小值运算符（Smooth-Minimum）或平滑并集运算符（Smooth-Union）。这与常规的最小值/并集运算符类似，但它是“平滑的”。这意味着，常规的最小值/并集运算符接受两个输入SDF（a和b），并返回其中距离最近的一个，从而有效地将两个形状合并成一个单一的场（合并后的形状的距离是到最接近部分的距离），而平滑版本则通过混合和熔化两个形状来结合它们，当它们足够接近时。这不仅有助于将形状聚合在一起，还能通过它们雕刻有机形状，就像原始体是由黏土制成的。

### 关于 Smooth-minimums
---
- 所以，常规的最小值运算符仅仅是选择两个SDF（a和b）中值较低的一个：
```c++
...
- 大多数语言都已经定义了这个函数，要么作为硬件指令（最有可能），要么通过软件模拟，并带有像上面代码中的实际分支（最不可能）。

- 现在，平滑最小值必须在a和b的值之间实现平滑的、非二进制的过渡，当a和b在某个容差k（以常规距离单位度量）内足够接近时。因此，当函数或表面彼此接近时，它们将融合或熔化在一起。

- 我们可以设计无限数量的此类函数，每个都有不同的优缺点，但以下是一些我曾经使用过的平滑最小值函数的列表：
...
float2 SMinCubic(float a, float b, float k)
{
    ...
    float m = h*h*h * 0.5;
    float s = m*k * (1.f/3.f);
    return (a<b)? float2(a - s, m) : float2(b - s, 1.f - m);
}
```
...
{
	float alpha = clamp(0.5 + (d2 - d1)/(2*k) , 0.0, 1.0);  //计算alpha
	return lerp(d2, d1, alpha) - k*alpha*(1.0 - alpha);  //线性插值 平滑调整
}
```


## 十. 利用Post Box 在后期屏幕上显示图片 .
