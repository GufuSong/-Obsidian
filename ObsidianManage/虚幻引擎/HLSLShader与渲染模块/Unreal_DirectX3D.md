...
- 假设四边形$A_1,A_2,A_3,A_4$ ,  以$A_1$为原点 ,  其他顶点逆时针排列进行计算 .  构成矢量A: (A1->A4); B:(A1->A2);
- 为了维持正常的四边形形态不会随机为菱形 ,  矢量A 与 矢量B 的夹角需要等于 90 度 .
- 达成面向摄影机的条件极为 A与B 的叉乘 与 Camera -> A_1构成的矢量 等于 180 度 或 0 度(需要开启双面渲染).
- A_3的位置是矢量A与B的 和.  (均计算相对位置)
