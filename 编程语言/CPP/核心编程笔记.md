...
- `override` 是 C++11 引入的一个**声明修饰符**（special identifier），用于标注派生类成员函数为**重写**基类中的虚函数或纯虚函数
...
- 可写作 `void f() override final;` 或 `final override` ([维基百科](https://en.wikipedia.org/wiki/C%2B%2B11?utm_source=chatgpt.com "C++11"))
...
>不用书写类型了, 编译器会自动推理类别 .  但凡是上下文无法推断类型的均无法使用此关键字
...
* `返回类型` operator1`运算符` (`参数列表`) 为运算函数名称, 底层原理为:
...
//类内声明双向运算符时,准许只有一个参数,因为默认情况下编译器会调用运算符左边的重载函数,并将运算符右边的对象传入函数 .
//如果你在类外声明一个二元运算符（如 `+`），却只给一个参数，那么编译器会直接报错。
```
>在声明解引用和乘法时 ,  依靠参数数量区分它们 .  解引用需要的参数永远比乘法少一个 .
...
* 运行结果为动物在说话, 原因为在编译阶段, 引用Animal &animal, 确定了函数地址. 如果想执行让猫说话, 那么这个函数地址就不能提前绑定, 需要在运行阶段进行绑定.
